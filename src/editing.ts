...
export function validRow(row: TimelineRow, track: Track, duration: number): string | null {
  if (row.start === null && row.end === null && (track === "lyrics" || track === "words")) return null;
  if (row.start === null || row.end === null || !Number.isFinite(row.start) || !Number.isFinite(row.end))
...
export function beatGrid(bpm: number, anchor: number, end: number, meter: number): TimelineRow[] {
  if (!Number.isFinite(bpm) || bpm < 20 || bpm > 400 || !Number.isFinite(anchor) || anchor < 0 || anchor >= end
      || !Number.isInteger(meter) || meter < 1 || meter > 12) throw new Error("BPMは20〜400、開始位置は曲の範囲内、拍子は1〜12で指定してください。");
