...
「声の表現」の編集画面で[Demucs / Mel-Bandの分離比較](docs/separation-comparison.md)ができます。原曲から作ったMel-Bandのボーカル・伴奏を既存ボーカルと同じ位置で試聴し、AST / YAMNetの検出結果も比較できます。既存トラックと手修正は保持します。

「声の表現」の編集画面から[AST / YAMNetの比較](docs/vocal-comparison.md)を実行できます。ハミング・ブレス・ビートボックスのスコアと候補を、原曲・分離ボーカル別に表示します。候補クリックで音声と試聴区間が切り替わります。通常の検出結果と手修正は保持します。YAMNetの準備は画面のセットアップまたは `npm run setup:yamnet` で行います。
