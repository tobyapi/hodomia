...
class Jobs:
    ...
    def start(self, root, options):
        ...
        if options.get('scope') not in ('vocal-events', 'vocal-comparison', 'separation-comparison') and not options.get('region') and not (self.runtime / 'chordmini/installation.json').is_file():
            raise ControlError('MODEL_NOT_READY', 'BTCモデルをセットアップしてください。')
        if options.get('scope') in ('vocal-comparison', 'separation-comparison') and not (self.runtime / 'yamnet/installation.json').is_file():
            raise ControlError('MODEL_NOT_READY', '比較用のYAMNetモデルをセットアップしてください。')
        if options.get('scope') == 'separation-comparison' and not (self.runtime / 'melband/installation.json').is_file():
            raise ControlError('MODEL_NOT_READY', 'Mel-Bandモデルをセットアップしてください。')
