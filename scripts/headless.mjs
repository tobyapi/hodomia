...
const runtime = runtimeIndex >= 0 ? args[runtimeIndex + 1] : args.find(a => a.startsWith('--runtime='))?.slice(10)
  ?? process.env.MUSIC_SWEEPER_RUNTIME ?? path.join(root, '.runtime');
const python = process.env.MUSIC_SWEEPER_PYTHON ?? path.join(runtime, 'venv', process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python');
